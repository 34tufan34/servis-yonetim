(function () {
  "use strict";
  if (window.__SYS_EXPERIENCE_V44837__) return;
  window.__SYS_EXPERIENCE_V44837__ = true;

  const $ = (selector, root = document) => root.querySelector(selector);

  function installStyles() {
    const style = document.createElement("style");
    style.id = "sys-experience-v44837";
    style.textContent = `
      ...
      #screen-driver .driver-current-main{min-width:0!important}#screen-driver .driver-current-main>div:nth-child(2){min-width:0!important}#screen-driver .driver-current-name{max-width:100%!important;overflow-wrap:anywhere!important;word-break:break-word!important;white-space:normal!important;font-size:clamp(27px,4.2vw,52px)!important}#screen-driver .driver-finish-btn.ready:not(.distribution){background:linear-gradient(135deg,rgba(185,28,28,.92),rgba(127,29,29,.96))!important;border-color:rgba(248,113,113,.65)!important;color:#fff!important;box-shadow:0 0 18px rgba(239,68,68,.13)!important}
    `;
  ...
  function install() { installStyles(); buildAiHub(); installContactImport(); showFirstSetup(); }
