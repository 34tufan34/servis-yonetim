(function () {
  "use strict";
  if (window.__SYS_EXPERIENCE_V44839__) return;
  window.__SYS_EXPERIENCE_V44839__ = true;

  const $ = (selector, root = document) => root.querySelector(selector);

  function installStyles() {
    const style = document.createElement("style");
    style.id = "sys-experience-v44839";
    style.textContent = `
  ...
  function buildAiHub() {
    ...
    screen.addEventListener("click", (event) => {
      ...
      window.setTimeout(() => { card.classList.remove("scanning"); if (stateLabel) stateLabel.textContent = "● ANALİZ TAMAMLANDI"; updatePersonnelSummary(); }, 1250);
    }, true);
