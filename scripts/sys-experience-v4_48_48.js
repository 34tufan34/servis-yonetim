(function () {
  "use strict";
  if (window.__SYS_EXPERIENCE_V44848__) return;
  window.__SYS_EXPERIENCE_V44848__ = true;

  const $ = (selector, root = document) => root.querySelector(selector);

  function installStyles() {
    const style = document.createElement("style");
    style.id = "sys-experience-v44848";
    style.textContent = `
  ...
  function buildPriceOffersPanel() {
    const nav = $(".nav");
    const main = $("#contentScroll");
    const financeNav = $('.nav-btn[data-module="finance"]');
    if (!nav || !main || !financeNav || $("#screen-price-offers")) return;
    const navButton = document.createElement("button");
    navButton.className = "nav-btn price-offers-nav";
    navButton.type = "button";
    navButton.innerHTML = '<span class="nav-label"><span class="dot"></span>Fiyat Teklifleri</span>';
    nav.insertBefore(navButton, financeNav);
    const screen = document.createElement("section");
    screen.className = "module-screen";
    screen.id = "screen-price-offers";
    screen.innerHTML = '<div class="price-offers-shell"><header class="price-offers-hero"><small>SYS · FİYAT TEKLİFLERİ</small><h2>KM Fiyat Teklifleri</h2><p>Firma, toplam kilometre, personel ve fiyat bilgilerini kaydet; kilometre fiyatını otomatik hesapla.</p></header><div id="priceOffersHost"></div></div>';
    main.appendChild(screen);
    navButton.addEventListener("click", () => {
      document.querySelectorAll(".nav-btn").forEach((button) => button.classList.remove("active"));
      document.querySelectorAll(".module-screen").forEach((item) => item.classList.remove("active"));
      ...
      if ($("#pageTitle")) $("#pageTitle").textContent = "Fiyat Teklifleri";
      if ($("#pageLead")) $("#pageLead").textContent = "KM bazlı iş tekliflerini hesapla, kaydet ve geçmiş kayıtları yönet.";
      $("#sidebar")?.classList.remove("open");
      $("#contentScroll")?.scrollTo({ top: 0, behavior: "smooth" });
    });
    document.addEventListener("click", (event) => {
      if (event.target.closest(".nav-btn") !== navButton) navButton.classList.remove("active");
    }, true);
  }

  function installKmOfferRecords() {
    const priceHost = $("#priceOffersHost");
  ...
  function install() { installStyles(); buildAiHub(); buildPriceOffersPanel(); installContactImport(); installCurrentLocationAddress(); installAddressBulkDelete(); installDiscountFuelSettings(); installKmOfferRecords(); showFirstSetup(); }
