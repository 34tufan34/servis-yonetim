(function () {
  ...
  if (window.__SYS_COMMAND_PANEL_V44845__) return;
  window.__SYS_COMMAND_PANEL_V44845__ = true;
  ...
  function makePanel() {
    ...
    screen.innerHTML = `
      <div class="command-preview-shell">
        <header class="command-preview-hero">
          ...
          <div class="command-preview-topline"><span>SYS · OPERASYON MERKEZİ</span><b>CANLI YÖNETİM</b></div>
    ...
    const shell = $(".command-preview-shell", screen);
    const operationsView = document.createElement("div");
    operationsView.id = "commandOperationsView";
    const hero = $(".command-preview-hero", shell);
    const body = $(".command-preview-body", shell);
    if (hero) operationsView.appendChild(hero);
    if (body) operationsView.appendChild(body);
    const tabs = document.createElement("div");
    tabs.className = "command-main-tabs";
    tabs.innerHTML = '<button class="command-main-tab active" type="button" data-command-tab="operations">Operasyon</button><button class="command-main-tab" type="button" data-command-tab="offers">KM Fiyat Teklifleri</button>';
    const offersView = document.createElement("section");
    offersView.id = "commandOffersView";
    offersView.hidden = true;
    offersView.innerHTML = '<div id="commandKmOfferHost"></div>';
    shell.append(tabs, operationsView, offersView);
    ...
    screen.addEventListener("click", (event) => {
      ...
      if (commandTab) {
        const showOffers = commandTab.dataset.commandTab === "offers";
        operationsView.hidden = showOffers;
        offersView.hidden = !showOffers;
        tabs.querySelectorAll("[data-command-tab]").forEach((button) => button.classList.toggle("active", button === commandTab));
        if ($("#pageLead")) $("#pageLead").textContent = showOffers ? "Firma, toplam kilometre, personel ve fiyat bilgileriyle tekliflerini hızlıca hesapla ve kaydet." : "Personel ve okul servislerini, saha durumunu ve mazot uyarılarını tek merkezden yönet.";
        $("#contentScroll")?.scrollTo({ top: 0, behavior: "smooth" });
        return;
      }
