(function () {
  ...
  async function requestRoute(context, force) {
    ...
    try {
      ...
      if (!route || !Number.isFinite(Number(route.distance))) throw new Error("Rota bulunamadı");
      if (serial !== requestSerial) return;
      routeResult = { distance: Number(route.distance), target, updatedAt: Date.now(), accuracy: Number(origin.accuracy || 0) };
  ...
  function startGps(force) {
    ...
    watchId = navigator.geolocation.watchPosition((position) => {
      ...
      latestPosition = null;
      routeResult = null;
      routeState = error?.code === 1 ? "Konum erişimi alınamadı · GPS / Rotayı Yenile" : "GPS sinyali alınamıyor";
      renderMetrics();
  ...
  function installStyle() {
    ...
    style.id = "sys-live-route-v44831-style";
    style.textContent = `
      .sys-live-route-host{grid-column:1/-1;width:100%;margin-top:9px}
      .sys-live-route{display:grid;grid-template-columns:1fr;gap:8px;padding:10px;border:1px solid rgba(56,189,248,.28);border-radius:14px;background:linear-gradient(135deg,rgba(14,165,233,.10),rgba(15,23,42,.38))}
