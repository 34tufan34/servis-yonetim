(function () {
  ...
  if (window.__SYS_V44815_SETTINGS_COMMAND_FIX__) return;
  window.__SYS_V44815_SETTINGS_COMMAND_FIX__ = true;
  ...
  function setFuelButtonLabels() {
    ...
    if (settingsButton) settingsButton.textContent = "Motorin Fiyatlarını Güncelle";
    const commandButton = $("#refreshFuelOfficialHtmlBtn");
    if (commandButton) commandButton.textContent = "Motorin Fiyatlarını Güncelle";
  }
  ...
  function installFuelRefresh() {
    if (window.__SYS_V44815_FUEL_REFRESH_INSTALLED__) return;
    window.__SYS_V44815_FUEL_REFRESH_INSTALLED__ = true;
    ...
    ["refreshFuelOfficialHtmlBtn", "refreshFuelPricesBtn", "refreshFuelOfficialHtmlSettingsBtn", "testFuelConnectionBtn"].forEach((id) => {
      ...
      if (!oldButton || oldButton.dataset.v44815Bound === "1") return;
      const button = oldButton.cloneNode(true);
      button.dataset.v44815Bound = "1";
  ...
  function install() {
    ...
    window.setInterval(updateAiDock, 3000);
  }
