(function () {
  "use strict";
  if (window.__SYS_EXPERIENCE_V44846__) return;
  window.__SYS_EXPERIENCE_V44846__ = true;

  const $ = (selector, root = document) => root.querySelector(selector);

  function installStyles() {
    const style = document.createElement("style");
    style.id = "sys-experience-v44846";
    style.textContent = `
  ...
  function buildAiHub() {
    ...
    screen.innerHTML = `<div class="sys-ai-hub">
      ...
      <div class="sys-ai-hub-actions">
        ...
        <button class="sys-ai-hub-action" data-action="km-offers"><b>09</b><strong>KM Fiyat Teklifleri</strong><span>Firma, kilometre, personel ve fiyat kayıtlarını hesapla ve yönet.</span></button>
      </div><section class="sys-ai-km-offers" id="sysAiKmOfferSection"><div id="sysKmOfferHost"></div></section><div class="sys-ai-hub-content" id="sysAiHubContent"><div class="sys-ai-hub-note">Okul güvenliği dâhil tüm akıllı denetimler artık bu merkezde toplanır.</div></div></div>`;
    ...
    screen.addEventListener("click", (event) => {
      ...
      if (!action) return;
      if (action.dataset.action === "km-offers") {
        event.stopImmediatePropagation();
        $("#sysAiKmOfferSection")?.scrollIntoView({ behavior: "smooth", block: "start" });
        return;
      }
  ...
  function installKmOfferRecords() {
    const sysHost = $("#sysKmOfferHost");
    const anchor = $("#discountFuelSettingsCard") || $("#fuelPriceSettingsCard");
    if ((!sysHost && !anchor) || $("#kmOfferRecordsCard")) return;
    state.settings = state.settings || {};
    ...
    if (sysHost) sysHost.appendChild(card); else anchor.after(card);
