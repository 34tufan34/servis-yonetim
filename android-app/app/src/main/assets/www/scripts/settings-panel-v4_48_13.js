(function () {
  ...
  if (window.__SYS_SETTINGS_PANEL_V44813__) return;
  window.__SYS_SETTINGS_PANEL_V44813__ = true;

  const STORAGE_KEY = "SYS_SETTINGS_PANEL_SECTIONS_V44813";
      ...
      `
        ...
        <div class="sys-settings-status-grid">
          <button type="button" class="sys-settings-status" data-open-group="system">
            <span>SÜRÜM</span><strong id="sysSettingsVersion">v4.48.13</strong><small>Çalışan uygulama</small>
          </button>
          <button type="button" class="sys-settings-status" data-open-group="system">
  ...
  function updateOverview() {
    const version = window.SYS_DISPLAY_VERSION || window.SYS_APP_VERSION || window.state?.appVersion || $("#appVersionDisplay")?.textContent || "v4.48.13";
    ...
    setText(versionTarget, compactText(version, "v4.48.13"));
  ...
  function installStyles() {
    if ($("#sysSettingsPanelV44813Styles")) return;
    const style = createElement("style");
    style.id = "sysSettingsPanelV44813Styles";
    style.textContent = `
      ...
      .sys-settings-nav {
        ...
        margin-bottom: 2px;
        background: linear-gradient(to bottom, var(--bg, #08090d) 74%, transparent);
        scrollbar-width: none;
      }
      .sys-settings-nav::-webkit-scrollbar { display: none; }
      .sys-settings-nav button {
        flex: 0 0 auto;
        min-height: 38px;
        padding: 8px 13px;
