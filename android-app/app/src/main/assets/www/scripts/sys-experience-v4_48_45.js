(function () {
  "use strict";
  if (window.__SYS_EXPERIENCE_V44845__) return;
  window.__SYS_EXPERIENCE_V44845__ = true;

  const $ = (selector, root = document) => root.querySelector(selector);

  function installStyles() {
    const style = document.createElement("style");
    style.id = "sys-experience-v44845";
    style.textContent = `
    ...
    style.textContent += `.address-book-row{grid-template-columns:22px 38px minmax(0,1fr) auto!important;gap:9px!important;min-height:0!important;padding:10px 12px!important;align-items:center!important}.address-book-row .address-bulk-select{margin:0!important;justify-self:center}.address-book-row .address-book-pin{width:38px!important;height:38px!important}.address-book-copy{min-width:0!important}.address-book-copy strong,.address-book-location,.address-book-note,.address-book-phone{max-width:100%;overflow-wrap:anywhere;word-break:break-word}.address-book-phone{display:inline-flex;color:#7dd3fc;text-decoration:none;font-size:11px;font-weight:800}.address-book-row-actions{display:flex!important;flex-wrap:nowrap!important;align-items:center!important;gap:6px!important}.address-book-row-actions .btn{white-space:nowrap!important;padding:7px 9px!important;font-size:10px!important}#commandKmOfferHost #kmOfferRecordsCard{margin:0!important;width:100%!important;max-width:none!important;padding:18px!important;background:rgba(8,15,27,.94)!important}#commandOffersView{padding:18px}.command-main-tabs{display:flex;gap:8px;padding:12px 16px;border-bottom:1px solid rgba(56,189,248,.16);background:rgba(3,8,18,.92)}.command-main-tab{padding:10px 14px;border:1px solid rgba(56,189,248,.2);border-radius:11px;background:rgba(15,23,42,.8);color:var(--muted);font-weight:900;cursor:pointer}.command-main-tab.active{border-color:rgba(56,189,248,.55);background:rgba(14,165,233,.16);color:#bae6fd}@media(max-width:700px){.address-book-row{grid-template-columns:20px 34px minmax(0,1fr)!important}.address-book-row .address-book-pin{width:34px!important;height:34px!important}.address-book-row-actions{grid-column:2/-1!important;justify-content:flex-start!important}.address-book-row-actions .btn{flex:0 1 auto!important;padding:7px 8px!important}.command-main-tabs{padding:9px}.command-main-tab{flex:1;padding:9px 7px;font-size:11px}#commandOffersView{padding:10px}}`;
  ...
  function installKmOfferRecords() {
    const commandHost = $("#commandKmOfferHost");
    const anchor = $("#discountFuelSettingsCard") || $("#fuelPriceSettingsCard");
    if ((!commandHost && !anchor) || $("#kmOfferRecordsCard")) return;
    state.settings = state.settings || {};
    ...
    if (commandHost) commandHost.appendChild(card); else anchor.after(card);
  ...
  function install() { installStyles(); buildAiHub(); installContactImport(); installCurrentLocationAddress(); installAddressBulkDelete(); installDiscountFuelSettings(); installKmOfferRecords(); showFirstSetup(); }
