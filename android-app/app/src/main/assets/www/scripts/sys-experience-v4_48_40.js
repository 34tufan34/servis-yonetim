(function () {
  "use strict";
  if (window.__SYS_EXPERIENCE_V44840__) return;
  window.__SYS_EXPERIENCE_V44840__ = true;

  const $ = (selector, root = document) => root.querySelector(selector);

  function installStyles() {
    const style = document.createElement("style");
    style.id = "sys-experience-v44840";
    style.textContent = `
  ...
  function buildAiHub() {
    ...
    const showAnalysis = (type) => {
      ...
      const data = {
        personnel: ["Personel Servisi Analizi", "Personel, firma, güzergâh ve aktif sefer bağlantıları birlikte tarandı.", [[activePeople,"Aktif personel"],[(state.companies||[]).length,"Firma"],[openPersonnel,"Aktif personel servisi"]]],
        school: ["Okul Güvenliği Analizi", "Öğrenci, veli, rehber ve okul servis oturumları güvenlik açısından incelendi.", [[activeStudents,"Aktif öğrenci"],[(state.guides||[]).length,"Rehber"],[openSchool,"Aktif okul servisi"]]],
        vehicle: ["Araç ve Evrak Analizi", "Araç kapasitesi, bakım kayıtları ve süresi yaklaşan ya da eksik evraklar kontrol edildi.", [[activeVehicles,"Aktif araç"],[(state.vehicleMaintenance||[]).length,"Bakım kaydı"],[missingDocuments,"Evrak uyarısı"]]],
        finance: ["Finans Radarı Analizi", "Tahsilat, ödeme, fatura ve mazot farkı sinyalleri finans riski için tarandı.", [[(state.financeRecords||[]).length,"Finans kaydı"],[unpaid,"Açık ödeme"],[$("#commandFuelPercent")?.textContent||"%0","Mazot değişimi"]]],
        missing: ["Eksik Servis Analizi", "Planlanmış seferler, kapanış durumları ve çetele uyumu eksik kayıtlar için tarandı.", [[incomplete,"Kontrol edilecek"],[(state.manualServiceCorrections||[]).length,"Düzeltme kaydı"],[(state.dayMarks||[]).length,"Çetele kaydı"]]]
      }[type];
      if (!data) return;
      analysisResult.innerHTML = `<small>SYS AI · ÖZEL ANALİZ</small><h3>${data[0]}</h3><p>${data[1]}</p><div class="sys-ai-analysis-metrics">${data[2].map(([value,label])=>`<div><strong>${value}</strong><span>${label}</span></div>`).join("")}</div>`;
    ...
    const updatePersonnelSummary = () => {
      ...
      set("#sysAiPersonnelCount", (state.personnel || []).filter((item) => item.status !== "Pasif").length);
